...
function setupMiniTerminal() {
    const demosSection = document.getElementById('interactive-demos');
    if (!demosSection) {
        // console.warn("setupMiniTerminal: interactive-demos section not found in DOM.");
        return;
    }

    const terminalOutput = demosSection.querySelector('#terminalOutput');
    const terminalInput = demosSection.querySelector('#terminalInput');
    const terminalContainer = demosSection.querySelector('#miniTerminal'); // For scrolling

    if (!terminalOutput || !terminalInput || !terminalContainer) {
        console.warn("setupMiniTerminal: One or more terminal elements (output, input, container) not found within #interactive-demos.");
        return;
    }
    ...
    terminalInput.value = ''; // Clear input field on setup
        ...
        help: () => {
            return `<p>Available commands:</p>
                    <ul>
                        ...
                        <li><code class="text-info">about</code> - Navigate to About section</li>
                        <li><code class="text-info">skills</code> - Navigate to Skills section</li>
                        <li><code class="text-info">projects</code> - Navigate to Projects section</li>
                        <li><code class="text-info">demos</code> - Stay on Demos section (or list demos)</li>
                        <li><code class="text-info">contact</code> - Navigate to Contact section</li>
                        ...
                        <li><code class="text-info">theme dark</code> - Switch to dark theme</li>
                        <li><code class="text-info">theme light</code> - Switch to light theme</li>
                        <li><code class="text-info">theme pentester</code> - Toggle pentester view</li>
                        <li><code class="text-info">easteregg</code> - Try it!</li>
                    </ul>`;
        },
        about: () => {
            navigateToSection('#about');
            return "<p>Navigating to About Me section...</p>";
        },
        skills: () => {
            navigateToSection('#skills');
            return "<p>Navigating to Technical Expertise section...</p>";
        },
        projects: () => {
            navigateToSection('#projects');
            return "<p>Navigating to Featured Projects section...</p>";
        },
        demos: () => {
            // Already on demos, or could list available demos if you add more
            navigateToSection('#interactive-demos'); // Ensures it's the active view
            return "<p>You are viewing Interactive Demos. Try the 'Vulnerability Scanner' or this 'Mini Terminal'.</p>";
        },
        contact: () => {
            navigateToSection('#contact');
            return "<p>Navigating to Contact section... You can also email: owuorvincent069@gmail.com</p>";
        },
        clear: () => {
            terminalOutput.innerHTML = '';
            return ""; // Return empty to prevent printing "undefined"
        },
        date: () => `<p>${new Date().toLocaleString()}</p>`,
        "theme": (args) => { // Handle "theme dark", "theme light", "theme pentester"
            const subCommand = args[0] ? args[0].toLowerCase() : null;
            const htmlElement = document.documentElement;
            const bodyElement = document.body;

            // Simplified applyTheme call from setupThemeToggles
            const applyTerminalThemeChange = (theme, isPentester = false) => {
                bodyElement.removeAttribute('data-pentester-view');
                if (isPentester) {
                    bodyElement.setAttribute('data-pentester-view', 'true');
                    htmlElement.setAttribute('data-bs-theme', 'dark');
                    localStorage.setItem('theme', 'pentester');
                    if(document.getElementById('lightModeIcon')) document.getElementById('lightModeIcon').style.display = 'inline-block';
                    if(document.getElementById('darkModeIcon')) document.getElementById('darkModeIcon').style.display = 'none';
                } else if (theme === 'dark') {
                    htmlElement.setAttribute('data-bs-theme', 'dark');
                    ...
                    if(document.getElementById('lightModeIcon')) document.getElementById('lightModeIcon').style.display = 'none';
                    if(document.getElementById('darkModeIcon')) document.getElementById('darkModeIcon').style.display = 'inline-block';
                }
                 // Re-render charts if skills section is currently loaded
                if (currentLoadedSectionId === 'skills' && typeof Chart !== 'undefined') {
                    setupSkillCharts();
                }
            };
            ...
            if (subCommand === 'dark') {
                applyTerminalThemeChange('dark');
                return "<p>Theme changed to Dark Mode.</p>";
            } else if (subCommand === 'light') {
                applyTerminalThemeChange('light');
                return "<p>Theme changed to Light Mode.</p>";
            } else if (subCommand === 'pentester') {
                const isPentesterActive = bodyElement.hasAttribute('data-pentester-view');
                if (isPentesterActive) {
                    const lastNormalTheme = localStorage.getItem('lastNormalTheme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
                    applyTerminalThemeChange(lastNormalTheme);
                    return "<p>Pentester view disabled. Reverted to previous theme.</p>";
                } else {
                    localStorage.setItem('lastNormalTheme', htmlElement.getAttribute('data-bs-theme') || 'light');
                    applyTerminalThemeChange(null, true);
                    return "<p>Pentester view enabled.</p>";
                }
            } else {
                return "<p>Usage: theme [dark|light|pentester]</p>";
            }
        },
        easteregg: () => `<p>Congratulations! <i class="fas fa-shield-alt text-success"></i> You found it!</p>`,
        default: (cmd) => `<p>bash: command not found: <span class="text-danger">${cmd}</span>. Type 'help'.</p>`
    };

    const handleTerminalInput = function(e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            const fullCommand = this.value.trim();
            const [commandName, ...args] = fullCommand.split(/\s+/); // Split command and arguments
            const commandExecutable = commandName.toLowerCase();
            this.value = '';

            const outputLine = document.createElement('p');
            outputLine.innerHTML = `<span class="terminal-prompt">guest@vince.portfolio:~$ </span>${fullCommand}`;
            terminalOutput.appendChild(outputLine);

            let responseHTML;
            if (commands[commandExecutable]) {
                responseHTML = commands[commandExecutable](args); // Pass arguments to command function
            } else {
                responseHTML = commands.default(commandExecutable);
            }
    ...
    // Ensure event listener is only added once, or remove previous if re-initializing
    terminalInput.removeEventListener('keydown', handleTerminalInput); // Remove if existing
    terminalInput.addEventListener('keydown', handleTerminalInput);

    const focusTerminalInputHelper = () => terminalInput.focus();
    terminalContainer.removeEventListener('click', focusTerminalInputHelper); // Remove if existing
    terminalContainer.addEventListener('click', focusTerminalInputHelper);

    terminalInput.focus(); // Auto-focus when section loads
}
